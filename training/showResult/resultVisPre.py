...
def showPic(args):
    ...
    centroid, error_total, nmi, ari, inertia_start, inertia_end, n_iter, labels_dict, ori_labels_dict = get_cluster_centroid(args,
                                                                                               args.best_model,
                                                                                               scaner,
                                                                                               scaner.get_data_num(),
                                                                                               save=args.save,
    ...
    # ind = ind[:, :400]
    # np.savetxt("./data/dist_"+str(args.expId)+".txt", ind)
    labels = {}
...
def get_cluster_centroid(args, model, data, data_num, center=None, feature=None, save=False,e=-1):
    ...
    centroids, error_total, nmi, ari, inertia_start, inertia_end, n_iter, cluster_data_neighbors, labels_dict = ClusterTool.DoKMeansWithError(path, args.sourcedata, k=args.clusterNum,
                                                                     n=data_num, center=center, feature=feature.cpu().data,save=save,epoch=e,tsne_filename="pre",labels=labels)

    return centroids, error_total, nmi, ari, inertia_start, inertia_end, n_iter, labels_dict, labels