...
# data_source = driver.CreateDataSource("./shp/spatio_split_moni_mian.shp")
...
if __name__ == '__main__':
    ...
    fig = go.Figure()
    #todo
    # layer = data_source.CreateLayer("spatio_split_moni_polygon", srs, ogr.wkbPolygon)
    ...
    # for k in list(cluster_labels.unique()):
        ...
        #     layer.CreateFeature(feature)
        # sample = df[df['cluster']==k].sample(frac=1,axis=0)
        # lats = sample['lat']
        # lons = sample['lon']
        # texts = sample['cluster']
        # fig.add_trace(go.Scattermapbox(
        #     name="spatio_split",
        #     lat=lats,
        #     lon=lons,
        ...
        #         size=4,
        #         color=colors[sample['cluster']],  # 'red',
        #         opacity=0.8,
